...
app = FastAPI(
    title = "Loan Prediction App using API - CI CD pipeline",
...
@app.get("/")
def index():
    return {"message": "Loan Prediction App using API - CI CD pipeline"}


@app.post("/prediction_status")
def predict(loan_details: LoanPrediction):
    data = loan_details.model_dump()
    prediction = generate_prediction([data])['predictions'][0]
    if prediction == "Y":
        pred= "Approved!!!"
    else:
        pred = "Rejected!!!"
...
def predict_gui(Gender: str,
    ...
    prediction = generate_prediction([data_dict])["predictions"][0]
    if prediction == "Y":
        pred= "Approved!!!"
    else:
        pred = "Rejected!!!"
